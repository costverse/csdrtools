...
```{r eval=FALSE, include=FALSE}
# --------------------- INSERT RESPONS EABOVE THIS CHUNK --------------------- 
```
</div>
...
wbs_without_0_units %>% 
  gt::gt() %>% 
  gt::tab_header(title = "Units by WBS", 
                 subtitle = "Includes Only WBS with Greather Than 0 Units") %>% 
  gt::tab_options(table.font.size = "8", data_row.padding = gt::px(1)) %>% gt::cols_align(c("left"), columns = c("WBSElementID",	"WBSElementName")) %>%
...
#### Average Labor Rate by Functional Element
...
The following table identifies the average labor rate At Completion by FunctionalElement for lowest level WBS elements.
...
#### Average Direct Enginnering Labor Rate To Date vs. At Completion
...
#### Average Direct Tooling Labor Rate To Date vs. At Completion
...
#### Average Direct Quality Control Labor Rate To Date vs. At Completion
...
#### Average Direct Manufacturing Labor Rate To Date vs. At Completion
...
csdr %>% 
  ...
  group_by(WBSElementID,
           WBSElementName) %>% 
  mutate(`Engineering Overhead Rate` = 
           sum(`Engineering Overhead_Dollars` / `Direct Engineering Labor_Dollars`, 
               na.rm = TRUE),
         `Manufacturing Operations Overhead Rate` = 
...
#### Average Direct Enginnering Overhead Rate To Date vs. At Completion
