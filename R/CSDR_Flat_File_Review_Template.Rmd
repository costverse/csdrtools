...
Provide the file name and path to the CSDR Flat File.  
```{r}
# Provide the filename (with extension) of the CSDR submission Flat File.
flat_file_source_name <- "FILE_NAME.xlsx"

# Provide the path to the CSDR submission Flat File (do not include the filename
# or extension).
flat_file_source_path <- here::here("data", "source")
```
...
dictionary <- dictionary %>% 
  rename("WBSElementID" = cwbs_level,
         "WBSElementName" = cwbs_element_name,
...
csdr %>% 
  ...
  gt::tab_options(table.font.size = "8", row.padding = gt::px(1))
```
...
2. Merge submission files into a tidy format.  

3. Confirm quantities and reported CSDR price match the contract.  

4. Confirm lowest level WBS elements units, hours, and costs are consistent with dictionary.  
...
5. Confirm units, hours, and dollars To Date vs. At Competition appears reasonable.  
...
6. Confirm units/hours/dollars Nonrecurring vs. Recurring appears reasonable.

* Are material costs primarily recurring?  
* Is engineering labor primarily non-recurring?  

7. Confirm labor rates by Functional Element appear reasonable.  
...
8. Confirm recurring unit cost by WSB Element appear reasonable.  

* Are the averages reasonable?  
*Are averages To Date reasonably compared to EAC/FAC.  

9. Do dictionary entries include the DID required info?  

10. Do WBS element and submission remarks include the required info?  

11. Are there any special requirements listed in the CSDR Plan that require checks?  

12 .Are there any other data deliverables (e.g., a BOM) that need to be checked for consistency with the CSDR submission (or could provide additional explanation)?  
